...
public class Config {
	...
	public static void readConfig(JavaPlugin plugin) {
		for(int a = 0; a < RegionCommand.pl.getConfig().getInt("regionsN"); a++) {
			Region region = new Region(RegionCommand.pl.getConfig().getString("regions." + a + ".regionName"), EventType.getEventTypeByName(RegionCommand.pl.getConfig().getString("regions." + a + ".eventType")), RegionCommand.pl.getConfig().getString("regions." + a + ".command"));
